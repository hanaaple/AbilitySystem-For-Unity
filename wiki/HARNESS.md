...
- **마일스톤 리마인드:** feature가 `✅ DONE` 될 때, 에이전트는 wiki가 낡았을 수 있음을 **한 줄 리마인드**한다(편집이 아니라 알림; `dev-docs/agent/HARNESS.md` §6 완료 처리 참조). 갱신 여부는 유저가 결정한다.
...
- wiki의 `todo` 섹션은 **2026-07-05 제거**됐다. 작업 추적은 `dev-docs/agent/feature-list.md`·`TODO-BOARD.md`가 담당하며, wiki는 설계 쇼케이스로만 유지한다. **요청 없는 한 todo 섹션을 다시 만들지 않는다.**

...
- **좁은 스코프·설계 갈림은 착수 전에 표면화·확인한다.** 구현이 특정 좁은 가정 위에서만 성립하거나, 스코프·정책이 여러 갈래로 갈리면, 한 쪽을 임의로 정하지 말고 그 한정성/갈림을 먼저 드러내 확인한다. (작업 프로세스 규약은 [`../agent/session-protocol.md`](../agent/session-protocol.md) '작업 중'.)
...
### 표현식 본문(`=>`) 지양 — 메서드는 블록 본문

메서드를 `=>` 표현식 본문으로 축약하지 않습니다. 블록 본문에 명시적 `return`을 씁니다. (유저 지시, 2026-08-21 — "왠만해선"이라 드문 예외는 허용하되 기본은 블록 본문.) 단 `IsValid => _field != null` 같은 **사소한 프로퍼티 접근자**는 `=>`를 허용합니다.
...
한 줄이 길어져도 여러 줄로 쪼개지 않고 한 줄로 유지합니다 — 메서드 시그니처(매개변수 많아도), 메서드 호출·체인, 조건식 등 문장 전반에 적용합니다. (가독성이 정말 무너지는 극단적 경우만 예외.) (유저 지시, 2026-08-25)
...
이 저장소의 코드는 사람과 AI 에이전트가 **둘 다** 읽고 수정한다. 관통 원칙 하나: **코드가 스스로 말하지 못하는 "왜"만, 낡지 않게, 그 자체로 완결되게 적는다.** 주석은 그것만 읽고 이해돼야 하고, 없어도 되면 없앤다(주석이 길어지면 코드가 불명확하다는 신호 — 주석보다 코드·이름을 먼저 고친다).
...
### 주석은 그 자체로 완결한다 — 외부 포인터 금지 (유저 지시, 2026-08-25)

주석은 **그것만 읽고 이해돼야** 한다. `→D8`·`INV-5`·`(UE: FAggregator)`처럼 **외부 문서·엔진 소스를 찾아봐야 뜻이 통하는 포인터는 코드 주석에 두지 않는다**(아무리 짧아도). 결정 근거·설계 이력은 `dev-docs/`(decisions.md·architecture 등)가 담당하고, 코드 주석은 그 자리에서 완결되는 "왜"만 적는다.

```csharp
// ❌ 외부를 찾아봐야 이해됨
// 단일 채널만 쓴다(→D10).
// mod 소유 이관 (UE: FActiveGameplayEffectsContainer::AddActiveGameplayEffect)

// ✅ 그 자체로 완결
// mod는 평가 채널로 나누지 않고 flat 리스트로 담는다 — 채널을 하나만 쓰기 때문.
```

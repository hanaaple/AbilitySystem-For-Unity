...
> **반영 기준:** feature `ability-system/attribute` @ 2026-08-21 (KST).

## Attribute 계층

- `AttributeSet`은 빈 추상 클래스(마커). 구체 Set이 `public AttributeData` 필드로 수치를 보유한다.
- `ResolvedGameplayAttribute` — 특정 Set의 특정 필드를 가리키는 불변 struct. 생성 시 `FieldInfo`를 캐싱하므로 **런타임 읽기/쓰기에 string 탐색이 없다**. `SetType + Name` 기반 `IEquatable`(Dictionary 키 사용). 직렬화·저작은 문자열 쌍 struct `GameplayAttribute`가 담고 `ToResolvedAttribute()`로 이 타입으로 해석한다 — UE 단일 `FGameplayAttribute`를 직렬화/런타임 둘로 나눈 것(C#은 `FieldInfo` 직렬화 불가).
- `AttributeData`(struct) — `BaseValue` / `CurrentValue` 쌍.
- 선언 규약: 구체 Set에 `public static readonly ResolvedGameplayAttribute Health = new(typeof(...), nameof(health))`. 필드명은 camelCase(`health`), 핸들명은 PascalCase(`Health`).

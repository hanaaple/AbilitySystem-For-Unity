# design.md — 전체 게임 기획 (보관 — 비활성)

> **비활성/보관.** GAS 데모 집중을 위해 잠시 치워둔 전체 게임 비전(비-GAS 인게임 부분). 되돌릴 때 활성 [design.md](../../project/design.md)로 통합한다.
> 현재 활성 방향(GAS 데모)은 [project/design.md](../../project/design.md).
...
**ItemSystem/EquipmentSystem의 구조는 확정된 것이 없다** — 2026-07-18 이전 설계·문서 전부 아카이브하고 **유저 주도로 무기 설계를 다시 시작**하는 중이다. 확정 요구사항·현황은 [`item-system/README.md`](../agent/feature/item-system/README.md). (아카이브는 열지 않는다.)

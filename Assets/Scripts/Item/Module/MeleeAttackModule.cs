...
{
    // 장비 시스템은 "공격"을 구체 모듈(MeleeAttackModule)이 아니라 이 능력으로 요구한다.
    // 총기 등 다른 공격 방식이 생겨도 시스템은 이 인터페이스만 알면 된다.
    public interface IWeaponAttack
    {
        // 명중 대상에게 적용할 데미지 효과. 모듈은 "무엇을" 적용할지 결정만 하고,
    ...
    // 근접 평타: 쿨다운·콤보 같은 per-item 상태가 없으므로 무상태 모듈이다.
    // 씬/GameObject를 만지지 않고 데미지 효과와 사거리라는 "결정"만 보유한다.
    [Serializable]

...
namespace Core.ItemSystem.Equipment
{
    ...
    // equipment D2: 특정 캐릭터 클래스(PlayerCharacter)에 의존하지 않는다 — 장비 상태가 캐릭터 수명(사망 등)과 얽히지 않게. (같은 GO의 ASC는 데미지 GE 적용에 참조.)
    // equipment D3: slot은 필드(딕셔너리)로 관리한다 — Module이 아님.
    // D10: 표현(소켓 소환)을 별도 컴포넌트로 쪼개지 않고 여기서 겸한다(장비=한 개념, 발견성 우위). 소켓 미지정 슬롯은 비주얼 없이 장착만.
    ...
    public class EquipmentComponent : MonoBehaviour
    {
        ...
        // D10: 부위별 프리팹 부착점. 인스펙터에서 손/등 등 Transform 지정(미지정 슬롯은 비주얼 없음).
        [SerializeField] private SocketBinding[] sockets;

        private AbilitySystemComponent _asc;

        // D3: 부위 → 장착된 아이템 인스턴스. per-item 상태는 ItemInstance가 소유(INV-5).
        private readonly Dictionary<SlotType, ItemInstance> _equipped = new();

        // D10: 부위 → 소켓 Transform(인스펙터 sockets에서 빌드) / 소환된 표현 오브젝트.
        ...
        // 장비 여부는 카테고리가 아니라 IEquippable 능력으로 판별한다(INV-11).
        public bool Equip(ItemInstance instance)
        {
            ...
            // 모듈 순회·상태 정렬은 ItemInstance가 담당(INV-5). 컨텍스트로 소유자 ASC를 넘긴다.
        ...
        // 모듈은 데미지 효과·사거리라는 "결정"만 주고(INV-3), 오버랩 감지·GE 적용은 여기(시스템)서 한다.
        ...
        // INV-11: 구체 타입이 아니라 능력 인터페이스로 공격 모듈을 찾는다.
        ...
        // D10: 장착 아이템의 표현 프리팹을 부위 소켓에 소환하고 ItemBehaviour를 인스턴스와 연결한다.
        // INV-4: 소환·연결 로직은 여기(시스템)에 있고 ItemBehaviour엔 없다. 프리팹/소켓 없으면 조용히 스킵.

...
namespace Core.ItemSystem.Module
{
    ...
    public abstract class ItemModule
    {
        ...
        // 순회·상태 정렬은 ItemInstance가 담당한다 — 상태 배열은 거기 캡슐화된다.
    ...
    {
        public sealed override IModuleState CreateState()
        {
            return new TState();
        }

        // IModuleState → 구체 상태 캐스트는 이 제네릭 베이스 한 곳에만 둔다.
        public sealed override void OnEquip(ModuleContext context, IModuleState state)
        {
            OnEquip(context, (TState)state);
        }

        public sealed override void OnUnEquip(ModuleContext context, IModuleState state)
        {
            OnUnEquip(context, (TState)state);
        }

...
#### dev-docs/agent/HARNESS.md
 - **`dev-docs/agent/HARNESS.md`는 agent 작동 시 필수 규약이자, 하위 규약·문서를 잇는 인덱싱 허브다** — 규약 문서 네이밍(`HARNESS.md` / `<파일>.harness.md`)·하위 폴더·상태 문서 인덱스가 여기 있다. 아래 시점에 읽고, 거기서 하위 문서로 진입한다.
